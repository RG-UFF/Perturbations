...
tr[ tensor_?ArrayQ ] := Tr[tensor, Plus, 2] 
...
partial[ tensor_ ] := Map[ D[tensor,#]&, coord ] 

dg = partial[ metric ]

Christoffel = inverse.(Transpose[dg,{2,1,3}]+Transpose[dg,{3,2,1}]-dg) / 2 

Chrfel = tr[Christoffel]

divergence[ vector_?ArrayQ ] := Inner[D,vector,coord,Plus,1] + Chrfel.vector 
...
RicciScalar := (
  ...
  RicciScalar = Tr[RicciTensor.inverse] ;
  Protect[ RicciScalar ];
  RicciScalar
  )
...
Einstein := (
  Unprotect[Einstein];
  Einstein = RicciTensor - RicciScalar metric / 2 ;
  Protect[ Einstein ];
  Einstein
  )
